...
// 식별자 : 절대적으로 실별할수 있는 고정 값 + 중복 없어야 한다.
// - !!! : 배열의 인덱스는 절대적인 식별자 역할을 할수 없다.
// - 주민등록번호 , 사번 , 학번 , 제품번호 , ISBN 등등
const 전국영화관 = [
    { 지점코드 : 1 ,지점명 : '안산센트럴락' , 주소 : '고잔로 108' , 
    영화 : [ { 영화코드 : 1 , 영화이름 : '서울의봄' , 시간테이블 :[{ 관: '1관', 시간: '12:00' } , { 관: '3관', 시간: '14:00' }]},
             { 영화코드 : 2 , 영화이름 : '노량' , 시간테이블 :[{ 관: '5관', 시간: '13:00' } , { 관: '4관', 시간: '16:00' }] }] 
    },
...
function 전국영화관출력(){
    ...
    let html =""
        ...
        for(let i = 0 ; i < 전국영화관.length ; i++){
            html += `<span style="margin: 10px" onclick="영화출력(${전국영화관[i].지점코드})">${전국영화관[i].지점명}</span><br/>`
        }
    // 3. 출력
    전국영화관출력구역.innerHTML = html;
}

function 영화출력(선택한지점코드){    // 함수선언 [ 실행조건 : 영화관 이름을 클릭했을때 ]
    // 1. 어디에
    ...
    for(let i = 0 ; i < 전국영화관.length; i++){
        ...
        if(전국영화관[i].지점코드 == 선택한지점코드){
            ...
            for(let j = 0 ; j<전국영화관[i].영화.length ; j++){
                html += `<span style="margin: 10px" onclick="시간출력(${선택한지점코드}, ${j})"> ${전국영화관[i].영화[j].영화이름} </span><br/>`
            }
        }
    }
    // 3. 출력
                ...
                for(let j = 0 ; j <전국영화관[i].영화.length ; j++){
                    if( 전국영화관[i].영화[j].영화코드 == 선택한지점코드 ){
                        ...
                        for( let t = 0; t < 전국영화관[i].영화[j].시간테이블.length ; t++){
                            html += `<span style="margin: 10px"> ${전국영화관[i].영화[j].시간테이블[t].관} 
